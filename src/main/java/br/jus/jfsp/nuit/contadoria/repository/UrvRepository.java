...
public interface UrvRepository extends JpaRepository<Urv, Long>{
	...
	Iterable<Urv> findAllByDataLessThanEqualAndDataGreaterThanEqual(Calendar data1, Calendar data2);
	...
	@Query("SELECT i FROM Urv i WHERE CAST(i.valor AS text) LIKE CONCAT('%',:like,'%') OR CAST(i.data AS text) LIKE CONCAT('%',:like,'%')")
	Page<Urv> findLikePage(Pageable pageable, @Param("like") String like);
